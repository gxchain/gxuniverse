...
const uint64_t      BIG_ROUND_SIZE          = 5;                        //一个大轮包含小轮数（5）
const uint64_t      ROUND_AMOUNT            = 40 * PRECISION;           //每一小轮的底池资产数（40GXC）
const uint64_t      ROUND_SIZE              = 5;                        //每一轮的参与人数（5）
const uint64_t      X                       = 20 * PRECISION;           //成为超级星需要抵押的资产数（20GXC）
const uint64_t      Y                       = 10 * PRECISION;           //成为小行星需要抵押的资产数（10GXC）
const uint64_t      Z                       = 11 * PRECISION;           //自激活需要抵押的资产数（11GXC）,需要再加上3个GXC的消耗
const uint64_t      DECAY_TIME              = 4 * 60;                   //衰减时间阈值，单位秒（4*3600s）
const uint64_t      DECAY_DURATION          = 1 * 60;                   //衰减时间间隔，单位秒（1*3600s）
const uint64_t      INIT_POOL               = 400 * PRECISION;          //初始化充值400GXC
...
const uint64_t      Z                       = 110 * PRECISION;          //自激活需要抵押的资产数（110GXC）,需要再加上3个GXC的消耗
